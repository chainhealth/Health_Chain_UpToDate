...
class EHRContract extends Contract {
    ...
    async _getPatientRecordData(ctx, firstName, lastName, email) {
        let queryString = {};
        queryString.selector = {'personalInformation.firstName':firstName, 'personalInformation.lastName':lastName, 'personalInformation.contactInformation.email': email};
    ...
    async getPatientRecord(ctx, firstName, lastName, email) {
        ...
        const patientDataJSON = await this._getPatientRecordData(ctx, firstName, lastName, email);
        if (!patientDataJSON) {
    ...
    async _updatePrescription(ctx, patientDataJSON, prescription) {
        try {
            ...
            await ctx.stub.putState(patientDataJSON.patientID, Buffer.from(stringify(sortKeysRecursive(patientDataJSON))));
        } catch (error) {
            return error;
        }
    }

    async writePatientPrescription(ctx, firstName, lastName, email, prescription) {
        const clientMSP = this._getMSPID(ctx);
        if (clientMSP !== 'InsuranceMSP') {
